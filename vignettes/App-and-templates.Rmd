...
templates are located in the `inst` folder in the package directory, but can also 
be downloaded directly from [github](https://github.com/LuddaLudwig/UPLforOAR/tree/main/inst). 
If you are running the app outside of the package installation directory, just 
make sure to keep the relative path to the [templates folder](https://github.com/LuddaLudwig/UPLforOAR/tree/main/inst/templates) 
with its contents consistent.
...
A couple quick notes about the shiny app and `UPLforOAR` package in general:
 ...
 - Be careful with data that are very small numbers or very close to, e.g. 99.9999997\%. 
 ...
 but it will need to be implemented by the user.
...
of the UPL calculations for both EG and NSPS for Normal, Lognormal, and Skewed. 
...
templates directory within the `UPLforOAR` installation to render the PDF. 
...
densities of emissions for each sources in your set of top performers for EG 
...
If you have less than 3 emission observations for the NSPS calculation those UPL 
results will be omitted. The Skewed function `Skewed_UPL()` requires greater 
than 3 observations to calculate the UPL. 

```{r fig7, out.width="80%", fig.cap="Observation density of EG emissions population."}
...
```{r fig8, out.width="80%", fig.cap="Observation density of NSPS emissions population."}
knitr::include_graphics("app_template_figs/fig8.png")
```

Lastly, a similar figure for the NSPS emissions population is plotted (assuming 
...
a table of all of the emissions data used in the EG UPL calculation. Appendix B 
is a table of all of the emissions data used in the NSPS UPL calculation. 
...
for its dependencies and any other packages loaded. 
...
blank reports included in the templates folder, such as Example_report.Rmd. 
...
the name of your source category. Much like the README.Rmd and vignettes, these 
templates include the analysis and text using an example emissions dataset. 
However, these templates are designed to be copied and then customized to the 
specific rule or pollutant, with the places where you might want to make changes 
pointed out. Often more than one hazardous air pollutant or multiple subcategories 
will be in the same report. In those cases, you will want to copy the relevant 
code blacks for each new analysis. Optional example text from past MACT floor 
analysis memorandums is included that can be expanded on or changed as needed. 
These also include appendices with the data, functions, and software version for 
reference. Having the code run inside the same document that produces the report 
is a clean way to prevent copy-paste mistakes or results becoming unlinked from 
