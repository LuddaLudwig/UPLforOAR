...
likelihood distribution. The colored probability line and areas indicate the 
predicted likelihood for the expected range of emissions, which by default is 
`seq(0,3*max(data$emissions,length.out=1024))`, but can be changed by supplying 
the `xvals` argurment to `Byaesian_UPL()`.
