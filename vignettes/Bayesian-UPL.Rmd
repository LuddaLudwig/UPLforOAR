...
analytically with `Normal_UPL()` using the mean, standard deviation, and *t*-statistic. 
...
moments to iteratively adjust the *t*-statistic until the desired significance is 
...
You will need to install [JAGS](https://sourceforge.net/projects/mcmc-jags/) 
before being able to use `Bayesian_UPL()` or `run_likelihood()`.
...
so there is no need to run `MACT_existing()` to select the data. The first step is to 
...
them being inside the `ggplot` object. See the `vignette("Density")` article for 
a thorough description of how `obs_density()` works.
...
ggplot()+
  ...
  geom_point(data = results$obs_pdf_dat,
            ...
            size = 3, alpha = 0.5, shape = 19, color = 'black')+
  geom_line(data = results$pred_pdf_dat,
            aes(x = x_hat, y = pdf_hat_med_med, color = distr), size = 0.75)+
  geom_area(data = results$pred_pdf_dat,
            aes(x = x_hat, y = pdf_hat_med_med, fill = distr), alpha = 0.5)+
...
Even though all of the observations are well below 1, this is an unnecessary 
constraint. Further information on distributions and their qualities that might 
help inform the best choice can be found in the `vignette("Distributions")` article. 
...
Below are the convergence plots for the parameters in the Gamma distribution 
that we fit to the IIS HCl emissions data. The left figures show the 3 MCMC 
chains (red, blue, and green lines) are well-mixed and without trend, indicating 
good convergence. The figures on the right are histograms of the posterior 
distributions of the Gamma parameters. Both indicate good convergence on values 
for the rate and shape. 
...
emissions `data` and manual prior options in `setup_likelihood()`, then execute 
`run_likelihood()` with options `future_runs`, `xvals`, and `maxY` if 
...
### - **`setup_likelihood()`:**
  ...
  MCMC search. You can also pass alternate RNG states in manually via the 
  `RNG.state` argument if you want to recreate a specific run other than the 
  default. Regardless of how the RNG is set, the state is always returned in the 
  output so the results are documented and reproducible.
  ...
  With manually provide priors, the initial values are set to be 10\% below the 
  upper bound and 10\% above the lower bound. Further information on using 
  manual priors can be found in the `vignette("Convergence-and-Priors")` article.
...
### - **`run_likelihood()`:**
  ...
  First, `run_likelihood()` will check if you have JAGS installed properly. Then 
  it will run the chosen likelihood distribution model, with the default or 
  ...
  the next 10,000 iterations. The other arguments include `minY` which sets the 
  lower limit of emissions and defaults to 0,  `maxY`, which sets the 
  ...
  probability. This is `seq(minY, maxY, length.out=1024)` by default. 
...
### - **`converge_likelihood()`:**
  ...
  `"Weak convergence"`. More details and examples on convergence can be found in 
  the `vignette("Convergence-and-Priors")` article. 
...
### - **`fit_likelihood()`:**
  ...
  details on this function can be found in the `vignette("Density")` article. The `obs_pdf` 
...
There might be circumstances where the `Bayesian_UPL()` function isn't flexible 
enough, and a customized option is needed. Below is an example of how you can 
write your own JAGS model script and use it with the `_likelihood()` functions. 
The JAGS model script below will create the file 'Custom_JAGS.R' in your working 
directory. This is a normal probability distribution with the same `pdf_obs`, 
`emission_hat`, and `pdf_hat_med` variables that are needed for calculating the UPL, 
assessing fit, and plotting the full probability function. However, we have changed 
the name of the parameters, set specific prior distributions, and manually 
specified to truncate the distribution between `(1000, 2000)`. 

```{r write_custom, echo = TRUE}
...
Next, we need to setup the likelihood model. We are going to simulate some 
emissions data that our custom model might work with. Since it is custom, we 
have to pass the path location and name of the JAGS script as a string, and also 
include lists of the parameter names to monitor that are new in our custom 
scripts as well as their initial starting values. The initial values need to be 
viable with regards to the prior distributions we customized (i.e. since 
`'parameter2'` cannot be greater than 100, setting an initial position `'parameter2' = 178`
would cause it to fail), organized as a list of three lists, with each of the 
three lists having different enough values that convergence can be assessed. 

```{r setup_custom, echo = TRUE}
set.seed(1)
...
Running the custom model works much the same once it has been setup. We will use 
the default settings: `minY = 0`, `maxY = 3*max(emissions)`, 
`xvals = seq(minY, maxY, length.out = 1024)`, and `future_runs = 3`.

```{r run_custom, echo = TRUE}
custom_run = run_likelihood(custom_setup)
```

The output and UPL calculations are fundamentally the same, the 99<sup>th</sup> 
percentile of the average of the distributions of future runs. The fitted probabilities
at the emission observations with confidence intervals and along `xvals` are 
returned as normal as well using `fit_likelihood()`. Since we know the data are 
normal and don't need bounds, we are also using a better kernel and bandwidth to 
match.

```{r output_custom, echo = TRUE}
custom_output = output_likelihood(custom_run)
custom_fit = fit_likelihood(custom_output, kernel = 'gaussian1', 
                            bw = "cv.ls")
```

This results in a UPL of `r signif(custom_fit$UPL_Bayes, 4)`, and we can plot 
the distributions below. Note that even though we gave very specific prior 
information in the form of a narrow normal distribution with a different mean in our 
custom model, the posterior distribution clearly still aligns with the data.

```{r custom_fig, echo = TRUE}
...
knitr::kable(conv_metrics,
             ...
             col.names = c("Distribution",'Parameter',"Diagnostic","Converged"))
```
And lastly, we can plot the convergence figures for our custom parameters. Here we need to specify we are using a custom distribution and again provide a list of the 
parameters to produce the plots for.

```{r custom_figs, echo = TRUE, fig.height = 4, out.width = "100%", fig.align = "center", results = FALSE}
