...
params:
  ...
  data:
    label: "Select dataset, with named columns of emissions and sources"
    input: file
    accept: ".csv"
...
```{r plot1, echo=FALSE, eval=source_fig, results=FALSE, fig.align = "center", fig.height=4, fig.cap="Figure 1. Observation density of emissions by source type. Source averages are indicated by vertical lines."}
...
ggplot()+
  ...
  geom_area(aes(y = pdf_n, x = x_hat, fill = 'b'), alpha = 0.25,
            data = pred_dat)+
  ylab("Density")+xlab("Chemical X emissions (units)")+
  ggtitle("Subcategory A existing source MACT floor test run population")+
  pop_distr_theme()+
...
ggplot()+
  ...
  geom_area(aes(y = pdf_n, x = x_hat, fill = 'b'), alpha = 0.25,
            data = pred_dat)+
  ylab("Density")+xlab("Chemical X emissions (units)")+
  ggtitle("Subcategory A new source MACT floor test run population")+
  pop_distr_theme()+
