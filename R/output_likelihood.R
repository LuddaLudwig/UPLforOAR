...
#' and `data`, `xvals`, and `future_runs` used as inputs to [run_likelihood()].
...
#' interquartile range, and the RNG.state for record keeping.
#' @description
...
output_likelihood = function(jags_model_run, significance = 0.99){
  ...
  if (distribution == "Skewed"){
    ...
    hat_quant = matrix(nrow = length(xi_quant), ncol = future_runs, data = NA)
