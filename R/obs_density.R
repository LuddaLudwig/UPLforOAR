...
#' @export
obs_density = function(data, emissions, xvals = NULL, up = Inf,
                       low = 0, kernel = 'gamma', bw = NULL){
  ...
  if (is.numeric(bw)){
    Obs_onPoint = np::npuniden.boundary(X = data_temp$emissions,
                                        Y = data_temp$emissions,
                                        a = low, b = up, proper = TRUE,
                                        kertype = kernel, h = bw)
    obs_den_df = np::npuniden.boundary(X = data_temp$emissions, Y = xvals,
                                       a = low, b = up, proper = TRUE,
                                       kertype = kernel, h = bw)
  } else if (is.character(bw)){
    Obs_onPoint = np::npuniden.boundary(X = data_temp$emissions,
                                        Y = data_temp$emissions,
                                        bwmethod = bw,a = low, b = up,
                                        proper = TRUE, kertype = kernel)
    obs_den_df = np::npuniden.boundary(X = data_temp$emissions, Y = xvals,
                                       a = low, b = up, proper = TRUE,
