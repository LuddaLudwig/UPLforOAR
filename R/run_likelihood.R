...
#' @description
#' Runs the JAGS model from write_likelihood() model_code output. Traces all variables in par_list running 3 chains in parallel with burnin and adapt of 10,000 each and keeping 10,000 iterations per chain.
#' @returns runjags object named run_results, likelihood distribution from the JAGS model script.
...
#' @param model_input results from write_likelihood(), including JAGS model script, distribution, initial values list, and parameters to monitor.
...
#' @param x_hat ordered sequence of emissions at which to predict probability density. Default is NULL, in which case x_hat is a 1024 length sequence between 0 and 3*max(data$emissions)
#'
run_likelihood=function(data,xvals=NULL,model_input,future_tests=3){
  ...
  rjm=suppressWarnings(runjags::run.jags(model=model_input$model_code,data=data_list,
