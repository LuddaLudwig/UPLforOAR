...
- xvals:
  ...
  Ordered sequence of emissions to define emission densities along.
...
A list containing two tibbles, `Obs_onPoint` with exact emission
observations and densities, and `obs_den_df` with densities for every
position given in `xvals`.

## References

Bandwidth is set following Renault O. & O. Scaillet. On the way to
recovery: A parametric bias free estimation of recovery rate densities.
